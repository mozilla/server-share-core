...
ax_attributes = dict(nickname='http://axschema.org/namePerson/friendly',
...
class AttribAccess(object):
    """Uniform attribute accessor for Simple Reg and
    Attribute Exchange values"""
...
def extract_openid_data(identifier, sreg_resp, ax_resp):
    ...
    pcard_map = {'first_name': 'givenName', 'middle_name': 'middleName',
                 'last_name': 'familyName', 'name_prefix': 'honorificPrefix',
...
class OpenIDResponder(object):
    ...
    def request_access(self, request, url, session):
        ...
        oidconsumer = consumer.Consumer(openid_session,
                self.openid_store, self.consumer_class)
